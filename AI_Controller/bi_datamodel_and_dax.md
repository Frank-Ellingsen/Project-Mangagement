...
```
*Budget at Completion: The total planned budget of WBS tasks.*
...
```
*Actual Cost (AC): Total combined actual costs logged on the project.*
...
```
*Returns the latest progress assessment for each task relative to the active date filter.*
...
```
*Earned Value (EV): The value of work physically completed (BAC * % Complete).*
...
CV = [EV] - [AC]
```
*Cost Variance (CV): Negative is over budget; positive is under budget.*

```dax
CPI = DIVIDE([EV], [AC], 1.0)
```
*Cost Performance Index (CPI): >1.0 is favorable cost efficiency; <1.0 is unfavorable.*

```dax
SV = [EV] - [PV]
```
*Schedule Variance (SV): Negative is behind schedule; positive is ahead of schedule.*
*(Note: PV is modeled as a cumulative budget line over time).*

```dax
SPI = DIVIDE([EV], [PV], 1.0)
```
*Schedule Performance Index (SPI): >1.0 is ahead of schedule; <1.0 is behind schedule.*
...
DIVIDE([BAC], [CPI], [BAC])
```
*Estimate at Completion (EAC): Assuming current cost efficiencies (CPI) persist until project end.*

```dax
EAC (Atypical) = 
[AC] + ([BAC] - [EV])
```
*Atypical EAC: Assuming future work will be completed precisely on budget.*

```dax
VAC = [BAC] - [EAC (Typical)]
```
*Variance at Completion (VAC): Expected deviation from budget at project end.*
...
* **Layout Grid**:
  ...
  * **Main Visual**: Cumulative S-Curve Chart.
    ...
    * *Visual Cue*: The gap between EV and AC immediately shows cash efficiency, and the gap between PV and EV shows schedule drift.
